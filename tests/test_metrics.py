...
def test_metric_event_logs_payload(capsys):
    metric_event("my.counter", value=2, type="counter", unit="n", tag="A")
    outer = read_last_json_line(capsys)
...
def test_inc_metric_sets_counter_type(capsys):
    inc_metric("requests.total", 3, route="/v1")
    inner = parse_inner_message(read_last_json_line(capsys))
...
def test_observe_metric_timer_type(capsys):
...
def test_timing_context_manager_emits_ms(capsys):
